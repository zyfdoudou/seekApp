<template>
	<view class="login">
		...
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="phoneData" type="text" maxlength="11" placeholder="联系电话"></wInput>
				<w-button text="发送短信验证" bgColor="red" @click.native="sendTel()"></w-button>
				<wInput v-model="passData" type="text" maxlength="11" placeholder="短信验证码"></wInput>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			sendTel() {
				var tel = RegExp(/^1[34578]\d{9}$/).test(this.phoneData);
				if (tel) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
					...
					//发送短信验证
					uni.request({
						url: "http://49.233.47.121:9898/sms/sms?tel=" + _self.phoneData,
						success: res => {
							console.log("结果是：" + res.data.code)
							_self.code = res.data.code;
						},
						fail: (res) => {
							console.log("fail is :" + res.data.code);
							_self.code = res.data.code;
						}
					});
					/**
					 * 在数据库中保存用户信息，并设置用户密码为666666
					 */

					/* this.tempNum = getNum(1000000);
					utils.sendSMS(this.phoneData,this.tempNum);
			...
			startLogin() {
				...
				*/
				if (this.passData == _self.code) {
					...
					uni.request({
						url: _self.url + "admin/seek/login",
						method: "POST",
						header: {
